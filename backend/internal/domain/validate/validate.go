...
// Rating enforces SPEC §4.2: 0.5–5.0 in 0.5 steps. Nil is valid (rating is
// optional per-check-in). The "in 0.5 steps" check uses scaled-mod-5 to
...
func Rating(r *float64) error {
	...
	if math.Mod(scaled, 5) != 0 {
		return fieldErr("rating must be in 0.5 steps")
	}
	return nil
}
