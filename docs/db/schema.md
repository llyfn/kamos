...
| `check_ins` | User logs. | `deleted_at` | Rating `NUMERIC(3,2)` 0.25..5.0 in 0.25 steps (widened from `NUMERIC(3,1)` / 0.5 steps in 004; floor lowered from 0.5 to 0.25 in 005), review ≤500, price coherence. Beverage immutable post-create (SPEC §4.4) — enforced at app layer. `edited_at TIMESTAMPTZ NULL` (003): rendering-only marker; set by the backend in the same TX as any tracked-field change. No CHECK, no index. |
...
| `check_ins.rating` `NUMERIC(3,2)` 0.25..5.0 step 0.25 (005) | §4.2, §6.2 |
