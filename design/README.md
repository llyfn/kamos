...
### Fixed elements
- Bottom tab bar is always fixed (Flutter `BottomNavigationBar` analogue): 5 tabs — **Feed · Lists · Discover · Notifications · Me**. All five tabs use the same hairline-icon style; there is no center FAB. The Notifications tab shows an unread dot (`--c-koh`, never a count) when any row is unread.
...
- 🟡 **Photo handling** — the kit shows placeholder tiles; real camera roll integration is Flutter's call. The 1-photo-on-submission cap is enforced UI-side in `CheckInScreen` and must also be enforced server-side per SPEC §4.1. Existing rows with up to 4 photos remain readable.
